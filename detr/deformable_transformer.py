...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 rectified_attention=0, dense_query=0, aps=0):
        super().__init__()
        ...
        self.decoder = DeformableTransformerDecoder(decoder_layer,
                                                    ...
                                                    d_model, rectified_attention, dense_query, aps)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None, attn_mask=None, seed_mask=None, cur_all_mask=1, pred_boxes=None):
        # self attention
        if self.aps is not None:
            tgt.detach_()
            src.detach_()
            tgt, attn_mask = self.aps(tgt, seed_mask.float().unsqueeze(-1), pred_boxes)

            attn_mask |= (~cur_all_mask).unsqueeze(-1) | (~cur_all_mask).unsqueeze(-2)
            ...
            attn_mask = (~attn_mask).repeat_interleave(8, 0)
        else:
            assert cur_all_mask == 1
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, d_model=256, rectified_attention=0, dense_query=0, aps=0):
        super().__init__()
        _layers = [copy.deepcopy(decoder_layer) for _ in range(num_layers)]
        self.aps = aps
        for i in range(num_layers - aps, num_layers):
            _layers[i].aps = RelationNet(d_model)
        for i in range(num_layers - rectified_attention, num_layers):
            _layers[i].cross_attn = RectifiedAttn(d_model)
    ...
    def forward(self, tgt: Tensor, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output, query_pos, reference_points_input, src, src_spatial_shapes, src_level_start_index, src_padding_mask,
                           attn_mask=attn_mask, seed_mask=seed_mask, cur_all_mask=cur_all_mask, pred_boxes=reference_points)

            # hack implementation for iterative bounding box refinement
            class_logits = self.class_embed[lid](output)
            ...
            if layer.aps is not None:
                ...
                intermediate_masks.append({"mask": cur_all_mask, "seed_mask": seed_mask})
            else:
            ...
            if lid >= self.num_layers - self.aps - 1:
                scores = prev_logits.max(-1).values.sigmoid()
                seed_mask = scores > config.score_thr
