...
class ImageBase(object):
    ...
    def clip_boader(self, odf, name):
        ...
        assert name in odf 
        x1, y1, width, height = odf[name]
        x2, y2 = x1 + width, y1 + height
        x1, y1 = np.maximum(x1, 0), np.maximum(y1, 0)
        x2, y2 = np.minimum(x2, self._width), np.minimum(y2, self._height)
        width, height = x2 - x1, y2 - y1
        odf[name] = [x1, y1, width, height]
        return odf
    ...
    def compare_caltech(self, thres):
        ...
        for i, dt in enumerate(dtboxes):
            ...
            for j, gt in enumerate(gtboxes):
                if gt.matched == 1:
                    maxiou_all = max(maxiou_all, dt.iou(gt))
                    continue
                if gt.ign == 0:
                    overlap = dt.iou(gt)
                    maxiou_all = max(maxiou_all, overlap)
                    if overlap > maxiou:
                        maxiou = overlap
                        maxpos = j
                else:
                    ...
                    if maxpos >= 0:
                        ...
                        if overlap > thres:
                            maxiou = overlap
                            maxpos = j
            dtboxes[i].maxiou_all = maxiou_all
            if maxpos >= 0:
